...
public final class ShardingSchema extends LogicSchema {
    ...
    public ShardingSchema(final String name, final Map<String, DataSourceParameter> dataSources,
                          final ShardingRuleConfiguration shardingRuleConfig, final boolean isCheckingMetaData,
                          final boolean isCachingTableMetadata, final boolean isUsingRegistry) {
        super(name, dataSources);
        shardingRule = createShardingRule(shardingRuleConfig, dataSources.keySet(), isUsingRegistry);
        metaData = createShardingMetaData(isCheckingMetaData, isCachingTableMetadata);
    }
    ...
    private ShardingMetaData createShardingMetaData(final boolean isCheckingMetaData, final boolean isCachingTableMetadata) {
        return new ShardingMetaData(getDataSourceURLs(getDataSources()), shardingRule, DatabaseType.MySQL, 
                ...
                GlobalRegistry.getInstance().getShardingProperties().<Integer>getValue(ShardingPropertiesConstant.MAX_CONNECTIONS_SIZE_PER_QUERY), isCheckingMetaData, isCachingTableMetadata);
    }
